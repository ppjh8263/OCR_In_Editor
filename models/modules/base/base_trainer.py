...
class BaseTrainer:
    ...
    def _save_checkpoint(self, epoch, log, save_best=False):
        ...
        if save_best:
            ...
            filename = os.path.join(self.checkpoint_dir, 'pren_best.pth.tar')
            torch.save(state, filename)
            self.logger.info("Saving current best: {} ...".format('pren_best.pth.tar'))
        else:
            state = {
                'arch': arch,
                'epoch': epoch,
            ...
            filename = os.path.join(self.checkpoint_dir,
                                    'pren-epoch{:03d}-loss-{:.4f}.pth.tar'.format(epoch, log['loss']))
            torch.save(state, filename)
