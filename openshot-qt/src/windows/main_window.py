...
from windows.views.vidCaps_listview import VidCapsListView
...
class MainWindow(updates.UpdateWatcher, QMainWindow):
    ...
    #여기를 통해서 사진이 전송
    def actionSendFrame_trigger(self, checked=True):
        log.info("actionSendFrame_trigger")
        ...
        # Prepare to use the status bar
        # 상태표시줄에 서버로 사진이 전송됨을 표시하게 하기 위한 status bar
        self.statusBar = QStatusBar()
        self.setStatusBar(self.statusBar)
        ...
        # (return is void, so we cannot check for success/fail here
        # - must use file modification timestamp)
        # img = openshot.Timeline.GetFrame(
    ...
    def actionSimple_View_trigger(self):
        ...
        self.tabifyDockWidget(self.dockEffects, self.dockEmojis, self.dockVidcap)
        self.showDocks([
            self.dockFiles,
            self.dockTransitions,
            self.dockEffects,
    ...
    def actionAdvanced_View_trigger(self):
        ...
        self.floatDocks(False)
        self.tabifyDockWidget(self.dockEmojis, self.dockEffects, self.dockVidcap)
        self.showDocks([
            self.dockFiles,
            self.dockTransitions,
            self.dockVideo,
    ...
    def setup_toolbars(self):
        ...
        # Add vidcaps toolbar
        self.vidCapsToolbar = QToolBar("vidCaps Toolbar")
        # 편집할 프레임을 전송
        self.vidCapsToolbar.addAction(self.actionSendFrame)
        # 편집된 프레임을 img로 뽑아내기(수정 요망)
        # self.vidCapsToolbar.addAction(self.actionSendFrame)
